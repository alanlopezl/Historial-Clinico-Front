import { Component } from '@angular/core';
import { RolesPackageService } from '../../roles/roles-package.service';
import { RolesInsertUpdateComponent } from '../../roles/roles-insert-update/roles-insert-update.component';
import { SweetAlertService } from 'src/app/services/sweet-alert.service';
import { BitacoraPackageService } from '../../bitacora/bitacora-package.service';
import { MatDialogRef } from '@angular/material/dialog';
import { EstadoPackageService } from '../estado-package.service';

@Component({
  selector: 'app-insert-update-estado',
  templateUrl: './insert-update-estado.component.html',
  styleUrls: ['./insert-update-estado.component.css']
})
export class InsertUpdateEstadoComponent {

  constructor(public _service: EstadoPackageService,
    public dialogref: MatDialogRef<InsertUpdateEstadoComponent>,
    private _sweet: SweetAlertService,
    private _bitacora: BitacoraPackageService
  ) { }

  ngOnInit(): void {
  }

  //limpia modal
  clear() {
    this._service.register.reset();
    this._service.inicializarForm();
  }

  //cerrarmodal
  cerrarmodal() {
    this.dialogref.close();
  }

  get validateOpinion(){
    return this._service.register.controls;
  }
  
  
  guardar() {


    if (this._service.register.valid) {

      if (!this._service.register.get('ID_ESTADO')?.value) {
       
        let datos = this._service.register.value;

        let params = {
          rol: datos.ESTADO,
        };

        this._service.crear(params).subscribe(resp => {
          if(!resp.ok){
            this._sweet.mensajeSimple(resp.msg,'ESTADOS','warning');
          }else{
            this._sweet.mensajeSimple('Creado correctamente', 'Estados', 'success');
            let params = {
              operacion:'INSERTO',
              fecha: new Date(),
              idusuario:localStorage.getItem('user'),
              tabla:'ESTADOS',
            }
            this._bitacora.crear(params).subscribe();
          }
          this._service.mostrar();
        });
        this.cerrarmodal();
      } else {
        // actualiza ususario
        let datos = this._service.register.value;

        let params = {
          id: datos.ID_ESTADO,
          rol: datos.ESTADO,
        };
        this._service.actualizar(params).subscribe((resp: any) => {
          if(!resp.ok){
            this._sweet.mensajeSimple(resp.msg,'ESTADOS','warning');
          }else{
          this._sweet.mensajeSimple('Actualizado correctamente', 'ESTADOS', 'success');
          let params = {
            operacion:'ACTUALIZO',
            fecha: new Date(),
            idusuario:localStorage.getItem('user'),
            tabla:'ESTADOS',
          }
          this._bitacora.crear(params).subscribe();
        }
          this._service.mostrar();
          this.cerrarmodal();
        });
      }
    }
  }
}
