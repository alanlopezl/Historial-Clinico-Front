...
export class PackageEnfermedadService {

  private enfermedad = new BehaviorSubject<any[]>([]);
  public response$: Observable<any[]> = this.enfermedad.asObservable();
  ...
  popForm(data:any){
   ...
   mostrar(busqueda: string=""){
    ...
    const request$ = this._globals.obtener('enfer?busqueda='+busqueda).pipe(tap((resp:any)=>{
    this.Cargando$.next(false);
     this.enfermedad.next(resp)
   }));
