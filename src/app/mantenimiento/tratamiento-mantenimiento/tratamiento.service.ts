...
export class TratamientoService {
  ...
  private tratamientos = new BehaviorSubject<any[]>([]);
  public response$: Observable<any[]> = this.tratamientos.asObservable();
  ...
  private url = `${environment.url}tratamiento`;
  ...
  popForm(data:any){
   ...
   mostrar(busqueda: string=""){
    ...
    const request$ = this._globals.obtener('tratamiento?busqueda='+busqueda).pipe(tap((resp:any)=>{
    this.Cargando$.next(false);
     this.tratamientos.next(resp)
   }));
