...
export class PackegeCuestionarioService {

  private cuestionario = new BehaviorSubject<any[]>([]);
  public response$: Observable<any[]> = this.cuestionario.asObservable();
  ...
  private url = `${environment.url}cuest`;
  ...
  popForm(data:any){
   ...
   mostrar(busqueda: string=""){
    ...
    const request$ = this._globals.obtener('cuest?busqueda='+busqueda).pipe(tap((resp:any)=>{
    this.Cargando$.next(false);
     this.cuestionario.next(resp)
   }));
